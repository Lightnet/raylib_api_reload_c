...
# Guide:
  ...
  Build file. Then use cmd line.
```
cmake --build . --target game
```
  To rebuild the dll library. Press R key and press enter on the terminal. It to prevent locks in game.dll. You can't overwrite it directly during rebuild—cmake --build would fail with "file in use."
